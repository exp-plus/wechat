...
class MiniProgram {
   ...
   * @return {Boolean} 内容是否安全
   */
  async messageSecurity({ content, access_token }) {
  ...
  async generateMiniProgramQRCode({ access_token,
    ...
    const { data } = await axios.post(`https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=${access_token}`, {
      ...
      page: page || 'pages/index/index', // 必须是已经发布的小程序存在的页面（否则报错），例如 pages/index/index, 根路径前不要填加 /,不能携带参数（参数请放在scene字段里），如果不填写这个字段，默认跳主页面
      width: 430,
